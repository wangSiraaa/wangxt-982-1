...
8. **资源组合与临时迁移**：多资源组合占用、冲突原因拆解、智能推荐替代房间、资源差异保留
...
| 审批与冲突 | 资源差异对比 | 换房时展示迁移前后的房间、设备、成本差异 |
| 审批与冲突 | 迁移历史 | 记录每次迁移的原因、时间、操作人、资源快照 |
...
### 3.4 资源组合与临时迁移流程
**多维度冲突检测**：预订时同时检测会议室、设备、前台接待、成本中心预算、布场缓冲、访客确认、安保审批等多重资源冲突，拆解冲突原因而非仅提示时间重叠
...
**智能推荐替代房间**：按容量匹配、楼层偏好、设备匹配度、成本差异、布场时间等多维度评分排序推荐

**迁移资源保留**：
- 迁移前后资源快照（房间、设备、成本中心、访客等）
- 资源差异对比（容量、楼层、费用、设备增减）
- 迁移历史记录（原因、时间、操作人、触发类型）
- 成本中心自动调整（差价计算与预算更新）
